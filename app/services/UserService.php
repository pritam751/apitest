...
class UserService extends ApiBaseService {
    ...
    public function makeUsersRecharge($userData, $deviceId, $accessToken) {
        $rechargeValidation = $this->rechargeValidation($userData);
       if ($rechargeValidation->fails()) {
            $rechageMessages = $rechargeValidation->messages();
            if ($rechageMessages->has('city')) {
                $this->messages['error'] = Config::get('constants.errorMessages.' . $rechageMessages->first('city'));
            }
            if ($rechageMessages->has('card_no')) {
                $this->messages['error'] = Config::get('constants.errorMessages.' . $rechageMessages->first('card_no'));
            }
            if ($rechageMessages->has('recharge_amount')) {
                $this->messages['error'] = Config::get('constants.errorMessages.' . $rechageMessages->first('recharge_amount'));
            }
        }
        
        $userArray = explode("--", base64_decode(base64_decode($accessToken)));
        $userId = end($userArray);
        $userData['device_id'] = $deviceId;
        //$this->validateData($userData);
        if (empty($this->messages['error'])) {
            $userRecharge = new UsersRecharge();
            $userRecharge->user_id = $userId;
            $userRecharge->city = $userData['city'];
            $userRecharge->card_no = $userData['card_no'];
            $userRecharge->recharge_amount =$userData['recharge_amount'];
            $userRecharge->created_at = date("Y-m-d H:i:s");
            $userRecharge->save();
            $lastId = $userRecharge->id;

            $userRechargeDetails = UsersRecharge::find($lastId);
            $this->messages['status'] = $this->successStatus;
            $this->messages['statusCode'] = 201;
            $this->messages['data']['Success'] = $userRechargeDetails->toArray();
        }
    ...
    public function rechargeValidation($userData) {
        $rules = array('city' => 'required', 'card_no' => 'required', 'recharge_amount' => 'required');
        $messages = array('city.required' => 'parameter_missing',
            'card_no.required' => 'parameter_missing',
            'recharge_amount.required' => 'parameter_missing');
